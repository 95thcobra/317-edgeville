...
public final class PlayerUpdating {
	...
	public static void update(Player player) throws Exception {
		...
		for (Iterator<Player> i = player.getLocalPlayers().iterator(); i.hasNext();) {
			Player other = i.next();
			if (other.getTile().isViewableFrom(player.getTile()) && other.getSession().getState() == IOState.LOGGED_IN && !other.isNeedsPlacement() && other.isVisible()) {
		...
		for (Player other : World.getPlayers()) {
			...
			if (other == null || other.equals(player) || other.getSession().getState() != IOState.LOGGED_IN)
				continue;
			if (other.getTile().isViewableFrom(player.getTile()) && other.isVisible()) {
	...
	private static void addPlayer(MessageBuilder out, Player player, Player other) {
		...
		out.putBit(true);
		out.putBit(true);
		Position delta = Position.delta(player.getTile(), other.getTile());
	...
	private static void updateLocalPlayerMovement(Player player, MessageBuilder out) {
		...
		if (player.isNeedsPlacement()) {
			out.putBit(true);
			int posX = player.getTile().getLocalX(player.getCurrentRegion());
			int posY = player.getTile().getLocalY(player.getCurrentRegion());
			appendPlacement(out, posX, posY, player.getTile().getZ(), player.isResetMovementQueue(), updateRequired);
		} else {
